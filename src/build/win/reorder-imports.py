...
def reorder_imports(input_dir, output_dir, architecture):
  ...
  input_image = os.path.join(input_dir, 'ninarium.exe')
  output_image = os.path.join(output_dir, 'ninarium.exe')
  ...
  for fname in glob.iglob(os.path.join(input_dir, 'ninarium.exe.*')):
...
def main(argv):
  ...
  parser.add_option('-i', '--input', help='reorder ninarium.exe in DIR',
      metavar='DIR')
  parser.add_option('-o', '--output', help='write new ninarium.exe to DIR',
      metavar='DIR')
